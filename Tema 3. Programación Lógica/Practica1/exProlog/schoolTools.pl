...
herramienta(udemy, [tipo(cursos), plataforma(web), nivel(universidad), nivel(profesionales), licencia(pago)]).
herramienta(google_classroom, [tipo(gestor), plataforma(web), nivel(primaria), nivel(secundaria), nivel(universidad), licencia(gratis)]).
