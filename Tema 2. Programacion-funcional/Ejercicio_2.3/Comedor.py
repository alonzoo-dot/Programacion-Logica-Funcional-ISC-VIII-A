...
#    R: Es una función que se puede usar como cualquier otro objeto, o sea, la puedes guardar en una variable,
#       pasarla como argumento a otra función o incluso devolverla.

#
# 2. ¿Cuál es la diferencia entre una función de orden superior y un callback?
#    R: Una función de orden superior es aquella que recibe funciones como argumento o devuelve una función.
#       Un callback es una función que se pasa como argumento para que otra función la ejecute después.

#
#
# 3. ¿Cuándo conviene usar comprensión de listas en lugar de un ciclo for?
#    R: Cuando quieres crear una lista de forma más corta y clara a partir de otra secuencia.
#       Sin tener que usar tanto código como en un for.


#
# 4. ¿Qué hace map() y cómo se relaciona con lambda?
#    R: map aplica una función a cada elemento de una secuencia y devuelve un iterador con los resultados.
#       Se relaciona con lambda porque muchas veces se usa una función lambda para definir operaciones rápidas.


#
# 5. ¿Qué ventaja ofrece pasar una función como argumento a otra función?
#    R: Permite reutilizar código, hacerlo más flexible y adaptar el comportamiento de una función sin modificar su estructura.
# ─────────────────────────────────────────────────────────────────────────────
...
#       R= tomar_orden(), porque recibe preparar_alimento que es una función, no un dato normal.
#
#      - ¿Dónde usarás comprensión de listas?
#       R= Dentro de tomar_orden() para generar la lista de porciones llamando al callback N veces.

#      - ¿Dónde usarás lambda + map()?
#       R= también dentro de tomar_orden() para asignar el precio a cada porción de la lista.
# ─────────────────────────────────────────────────────────────────────────────
...
def tomar_orden(preparar_alimento, cantidad, precio_unitario):
    ...
    porciones = [preparar_alimento() for i in range(cantidad)]
    ...
    # Desafio 1. Usa sum() y map() + lambda para calcular el TOTAL a pagar de cada orden.
    total_pagar = sum(map(lambda x: x, precios))

    # d) Devuelve los tres valores
    return porciones, precios, promocion, total_pagar


# Desafio 2. Crea una función elegir_producto(nombre) que sea de ORDEN SUPERIOR:
#    recibe un string ("pizza", "agua" o "tamal")
...
# Desafio 3. Usa la función del punto 2 para reemplazar los argumentos directos en las llamadas a tomar_orden().
